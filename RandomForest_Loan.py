...
def infoGain(data, featCol, labels):
    ...
    for v in val:
        split = data.loc[data[colName]==v, 'Loan_Status']
...
def buildTree(data, featNames, target): 
    ...
    if bestFeat == '':
        ...
        if isinstance(data.iloc[0][bestFeat], np.number):
            ...
            leftTarget = data.loc[data[bestFeat] <= avg, 'Loan_Status'] 
            rightTarget = data.loc[data[bestFeat] > avg, 'Loan_Status'] 
            ...
            for v in values: 
                ...
                target_child = data.loc[data[bestFeat] == v, 'Loan_Status']
...
def kFoldValidation(nTree):  
    ...
    #read the data file
    df = pd.read_csv(r"C:\Users\memoh\OneDrive\Desktop\Spring23\589\final_project\datasets\loan.csv")
    
    df = df.drop('Loan_ID', axis=1)
    
    # # Select the categorical columns to be one hot encoded
    # categorical_cols = ['Gender', 'Married', 'Dependents', 'Education', 'Self_Employed', 'ApplicantIncome', 'CoapplicantIncome', 'LoanAmount', 'Loan_Amount_Term', 'Credit_History', 'Property_Area']
    
    df['Loan_Status'] = df['Loan_Status'].map({'Y': 1, 'N': 0})
    
    # Split the dataframe based on the class column
    dfClass1 = df[df["Loan_Status"] == 0]
    dfClass2 = df[df["Loan_Status"] == 1]
    ...
    for i in range(10):
        ...
        actualLabel = testFoldAll.iloc[:,11]
...
def main():
    ...
    plt.title('Metrics Plot Loan Dataset')
