...
df = pd.read_csv(r'/Users/venkatasamyuktamalapaka/Downloads/predictive_maintenance.csv')
...
data['Failure Type'] = data['Failure Type'].replace({'Power Failure': 1, 'No Failure': 0,'Heat Dissipation Failure':2,'Overstrain Failure':3,'Tool Wear Failure':4,'Random Failures':5})


# Perform one-hot encoding on selected columns
df_encoded = pd.get_dummies(data, columns=columns_to_encode,dtype=int)


print(df_encoded,'fruit8')
...
def k_folds(data_to_load):
 ...
 grouped = data_to_load.groupby(['Failure Type'])
 print(grouped,'sam')

 for i in range(10):
    fold = {}
    for j in range(0,fold_size,6):
        index_0 = randrange(len(grouped.get_group(0)))
        index_1 = randrange(len(grouped.get_group(1)))
        index_2 = randrange(len(grouped.get_group(2)))
        index_3 = randrange(len(grouped.get_group(3)))
        index_4 = randrange(len(grouped.get_group(4)))
        index_5 = randrange(len(grouped.get_group(5)))
        fold [j] = ((grouped.get_group(0)).iloc[index_0].values)
        fold [j+1] = ((grouped.get_group(1)).iloc[index_1].values)
        fold [j+2] = ((grouped.get_group(0)).iloc[index_2].values)
        fold [j+3] = ((grouped.get_group(1)).iloc[index_3].values)
        fold [j+4] = ((grouped.get_group(0)).iloc[index_4].values)
        fold [j+5] = ((grouped.get_group(1)).iloc[index_5].values)
...
def main():
    ...
    for k in range(1,11,2):
        ...
        for m in range(1,11):
            ...
            y_train = train['Failure Type'][:].values
            y_test = test['Failure Type'][:].values
            X_train = train.drop(['Failure Type'], axis=1)[:].values
            X_test = test.drop(['Failure Type'], axis=1)[:].values
            # print(y_train)
            # print("-----------------------------------")
            # print(y_test)
            # print("-----------------------------------")
    ...
    plt.title('Line plot with error bars of Accuracy of Training data on Predictive Maintainance dataset')
    ...
    plt.xlabel('K-value')
    plt.ylabel('Average accuracy')
    plt.title('Line plot with error bars of Accuracy of Testing data on Predictive Maintainance dataset')
    ...
    plt.title('Line plot of F1 score of Training data on Predictive Maintainance dataset')
    ...
    plt.xlabel('K-value')
    plt.ylabel('Average F1 score')
    plt.title('Line plot of F1 score of Test data on Predictive Maintainance dataset')
