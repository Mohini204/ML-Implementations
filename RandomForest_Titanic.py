...
def infoGain(data, featCol, labels):
    ...
    for v in val:
        split = data.loc[data[colName]==v, 'Survived']
...
def buildTree(data, featNames, target): 
    ...
    if bestFeat == '':
        ...
        if isinstance(data.iloc[0][bestFeat], (int,float)):
            ...
            leftTarget = data.loc[data[bestFeat] <= avg, 'Survived'] 
            rightTarget = data.loc[data[bestFeat] > avg, 'Survived'] 
            ...
            for v in values: 
                ...
                target_child = data.loc[data[bestFeat] == v, 'Survived']
...
def kFoldValidation(nTree):  
    ...
    #read the data file
    df = pd.read_csv(r"C:\Users\memoh\OneDrive\Desktop\Spring23\589\final_project\datasets\titanic.csv")
    ...
    dfClass1 = df[df["Survived"] == 0]
    dfClass2 = df[df["Survived"] == 1]
    ...
    for i in range(10):
        ...
        actualLabel = testFoldAll["Survived"]
...
def main():
    ...
    plt.title('Metrics Plot Titanic Dataset')
