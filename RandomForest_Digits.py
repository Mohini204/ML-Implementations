...
def infoGain(data, featCol, labels):
    ...
    for v in val:
        split = data.loc[data[colName]==v, 'class']
...
def buildTree(data, featNames, target): 
    ...
    if bestFeat == '':
        ...
        if isinstance(data.iloc[0][bestFeat], np.number):
            ...
            leftTarget = data.loc[data[bestFeat] <= avg, 'class'] 
            rightTarget = data.loc[data[bestFeat] > avg, 'class'] 
            ...
            for v in range(113,115): 
                ...
                target_child = data.loc[data[bestFeat] == v, 'class']
...
def kFoldValidation(nTree):  
    ...
    class_dfs = [df[df['class'] == i] for i in range(10)]
        
    
    #split each dataset into 10 parts
    class0Parts = np.array_split(class_dfs[0], 10)
    class1Parts = np.array_split(class_dfs[1], 10)
    class2Parts = np.array_split(class_dfs[2], 10)
    class3Parts = np.array_split(class_dfs[3], 10)
    class4Parts = np.array_split(class_dfs[4], 10)
    class5Parts = np.array_split(class_dfs[5], 10)
    class6Parts = np.array_split(class_dfs[6], 10)
    class7Parts = np.array_split(class_dfs[7], 10)
    class8Parts = np.array_split(class_dfs[8], 10)
    class9Parts = np.array_split(class_dfs[9], 10)
    ...
    for i in range(10):
        ...
        testFoldAll = pd.concat([class1Parts[i], class2Parts[i], class3Parts[i], class4Parts[i], class5Parts[i], class6Parts[i], class7Parts[i], class8Parts[i], class9Parts[i]])
        testFold = testFoldAll.iloc[:, :-1].values
        
        actualLabel = testFoldAll.iloc[:,64]
        
        # concatenate rest of the parts to make the training set
        trainFold = pd.concat([pd.concat(class0Parts[:i]+class0Parts[i+1:]), pd.concat(class1Parts[:i]+class1Parts[i+1:]), 
                               pd.concat(class2Parts[:i]+class2Parts[i+1:]), pd.concat(class3Parts[:i]+class3Parts[i+1:]), 
                               pd.concat(class4Parts[:i]+class4Parts[i+1:]), pd.concat(class5Parts[:i]+class5Parts[i+1:]),
                               pd.concat(class6Parts[:i]+class6Parts[i+1:]), pd.concat(class7Parts[:i]+class7Parts[i+1:]),
                               pd.concat(class8Parts[:i]+class8Parts[i+1:]), pd.concat(class9Parts[:i]+class9Parts[i+1:])])
...
def main():
    ...
    plt.title('Metrics Plot Digits Dataset')
